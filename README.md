...
featurework.py
日期特征、缺失值填充、异常值处理
收盘/开盘比率、最高/最低比率、日收益率
3/5/10日的移动平均线和标准差
train.py
LightGBM模型
使用Optuna进行30轮参数搜索
3折时间序列分割
价格差异、波动率、动量、均值回归等进行特征增强
test.py

#结果说明
20支能预测正确3-5支，第一名预测正确5支

#题目说明
https://www.heywhale.com/home/competition/6826a9d0cc560e683810bf77/content/2
赛题&数据
一、赛题背景
基于历史数据预测未来股价涨跌
...
4.数据及文件下载
数据名称	数据描述	下载链接
训练数据（train.csv）	线上赛A阶段训练数据	训练数据（train.csv）
推理数据（test.csv）	线上赛A阶段推理数据	推理数据（test.csv）
baseline（基准代码）	本赛题baseline	基准代码（baseline）
提交样例	线上赛A阶段提交结果样例	提交样例（result.csv）
三、提交结果
...
4.提交异常：没有按照要求提交result，如：少提交、重复提交相同股票代码编号等，则会赋分-100分，选手可自行检查结果文件进行修改。（错误提交会占用每日提交次数）
五、其他说明
